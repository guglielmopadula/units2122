...
kFoldSsvm<-function(mltrain, cost, gamma ){
...
TuneSsvm<-function(mltrain,gamma){
  errold=1
  a=c(0,0)
  v=numeric(50)
  j=1
  for (i in seq(25,75)/50){
    err=kFoldSsvm(mltrain,i, gamma)
    v[j]=err
    j=j+1
    if (err<errold){
      a=i
...
TesterrorSsvm<-function(mldata,gamma, index) {
  #index=sample(nrow(mldata), size=40)
  mldatatrain=mldata[-index,]
  mldatatest=mldata[index,]
  a=TuneSsvm(mldatatrain,gamma)
  ssvm=svm(V1~., data=mldatatrain, kernel='sigmoid', gamma=gamma, cost=a$cost)
  predtrain=predict(ssvm,data=mldatatrain)
  ...
  predtest=predict(ssvm,newdata=mldatatest)
  ...
  print(paste('Sigmoid svm:', 'error (train) is', errtrain,'auc (test) is', auctrain,'error (test) is', errtest, 'auc (test) is', auctest, 'cost', a$cost,'gamma', gamma))
  return(a$vector) 
}
